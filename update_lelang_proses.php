    ...
    if($ukuran_file <= 100000000){
        if(move_uploaded_file($tmp_file, $path)){
            if(date('Y-m-d')>$tanggal_lelang || date('Y-m-d')>$waktu_akhir){
                ...
                $query = "UPDATE lelang SET padi_id_padi=?,user_id_user=?,nama_lelang=?,deskripsi=?,tanggal_lelang=?,waktu_berakhir=?,harga_awal=?,berat=?,foto_padi=? WHERE id_lelang = $id_lelang";
                $statement = $database->prepare($query);
                $statement->bind_param("iissssdds", $jenis,$id,$nama_lelang,$deskripsi,$tanggal_lelang,$waktu_akhir,$harga_awal,$berat,$nama_file);
                $statement->execute();
            ...
            echo '<script language="javascript">alert("Terjadi Kesalahan saat mengupload"); document.location="my_lelang.php";</script>';        
        }
    }else{
        echo '<script language="javascript">alert("Ukuran gambar terlalu besar"); document.location="my_lelang.php";</script>';
    }
}else{
    echo '<script language="javascript">alert("Tipe gambar harus Jpg/png"); document.location="my_lelang.php";</script>';
}
